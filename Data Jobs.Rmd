...
new_data <- data %>%
  ...
  filter(!grepl("United States", Location),
         Company!="CyberCoders") %>%
  distinct(Title, Company, Location, Description, .keep_all = T) %>% 
...
DT::datatable(select(table_data, -Link, -ID),
              ...
              options = list(pageLength = 25,
                             ...
                             columnDefs = list(
                               list(
                                 targets = c(7), #ie, Description column
                                 render = JS(
                                       "function(data, type, row, meta) {",
                                       "return type === 'display' && data.length > 360 ?",
                                       "'<span title=\"' + data + '\">' + data.substr(0, 360) + '...</span>' : data;",
                                       "}"))),
                             scrollY=F), # This allows for proper  veical sizing
              callback = JS('table.page(3).draw(false);'),
              caption = "Table includes jobs from the previous 30 days. Hover cursor over Description for full text.")  %>%
  formatStyle(c("Search","Program","Date", "Title", "Company", "Location", "Description"), "vertical-align"="top")

# DT::datatable(select(table_data, -ID, -Link, -Description),   
#               options = list(pageLength = 25,
#                              scrollY = F),
#               escape = F,
#               rownames = F,
#               caption = "Table includes only jobs from the previous 30 days")
